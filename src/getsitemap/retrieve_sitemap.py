...
def _concurrent_thread_starter(urls: list, thread_max: int, allow_xml_inference: bool):
    """
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=thread_max) as executor:
        processes = [executor.submit(_get_individual_sitemap, url, allow_xml_inference) for url in urls]
...
def _get_individual_sitemap(root_url: str, thread_max: int, allow_xml_inference: bool = True) -> dict:
    """
    ...
    if parsed_file.find("sitemapindex"):
        ...
        all_urls.update(_concurrent_thread_starter(sitemap_urls, thread_max, allow_xml_inference))
    else:
...
def retrieve_sitemap_urls(root_page: str, as_flat_list: bool = True, allow_xml_inference: bool = True, thread_max: int = 20) -> Union[list, dict]:
    """
    ...
    if sitemap_urls:
        ...
        new_urls = _concurrent_thread_starter(unique_sitemaps, thread_max, allow_xml_inference)
